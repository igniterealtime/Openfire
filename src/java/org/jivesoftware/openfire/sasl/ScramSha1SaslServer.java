...
//        String authzId = m.group(4);
        ...
        serverFirstMessage = String.format("r=%s,s=%s,i=%d", nonce, DatatypeConverter.printBase64Binary(getSalt(username)), ScramUtils.ITERATIONS);
...
//        String channelBinding = m.group(2);
        ...
        try {
            ...
            byte[] saltedPassword = getSaltedPassword(username);
            ...
            if (!Arrays.equals(clientProof, DatatypeConverter.parseBase64Binary(proof))) {
                throw new SaslException("Authentication failed");
            }
            byte[] serverKey = ScramUtils.computeHmac(saltedPassword, "Server Key");

            return ("v=" + DatatypeConverter.printBase64Binary(ScramUtils.computeHmac(serverKey, authMessage)))
   ...
   /**
    ...
    private byte[] getSalt(final String username) {
		try {
            ...
            return DatatypeConverter.parseBase64Binary(user.getSalt());
    ...
    /**
     * Returns the salted password stored when the password is initially set.
     * 
     * This affords us the opportunity of not storing a plaintext password.
     */
    private byte[] getSaltedPassword(final String username) throws SaslException {
		try {
			User user = UserManager.getUserProvider().loadUser(username);
			
	        return DatatypeConverter.parseBase64Binary(user.getHi());
		} catch (UserNotFoundException e) {
            throw new SaslException("Authentication Failed");
		}
    }
