...
public class CacheFactory {
     ...
     */
    private static Map<String, Cache<? extends Serializable, ? extends Serializable>> caches = new ConcurrentHashMap<>();
    ...
    */
    ...
    private static Cache getCacheByProperty( String property )
    {
        ...
        for ( final Map.Entry<String, Cache<? extends Serializable, ? extends Serializable>> entry : caches.entrySet() )
        {
    ...
    public static Cache[] getAllCaches() {
        List<Cache> values = new ArrayList<>(caches.values());
        return values.toArray(new Cache[0]);
    }
    ...
    public static synchronized void destroyCache(String name) {
        Cache<? extends Serializable, ? extends Serializable> cache = caches.remove(name);
        if (cache != null) {
     ...
     */
    @Deprecated
    public static synchronized Lock getLock(Serializable key, Cache<? extends Serializable, ? extends Serializable> cache) {
    ...
    private static ClassLoader getClusteredCacheStrategyClassLoader() {
        ...
        final Optional<PluginClassLoader> optionalPluginLoader = pluginManager.getPluginByName("Hazelcast Plugin")
            .map(pluginManager::getPluginClassloader);
        if (optionalPluginLoader.isPresent()) {
            final PluginClassLoader pluginLoader = optionalPluginLoader.get();
