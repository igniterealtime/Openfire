...
/**
 * Unit tests that verify the implementation of {@link ScramSha256SaslServer}
 *
...
public class ScramSha256SaslServerTest extends AbstractScramSaslServerTest
{
    ...
    {
        authFactory.when(() -> AuthFactory.getSalt(anyString(), eq(ScramSha256SaslServer.MECHANISM_NAME))).thenReturn(ScramSha256TestFixtures.SALT);
        authFactory.when(() -> AuthFactory.getIterations(anyString(), eq(ScramSha256SaslServer.MECHANISM_NAME))).thenReturn(ScramSha256TestFixtures.ITERATIONS);
        authFactory.when(() -> AuthFactory.getPassword(any())).thenReturn(ScramSha256TestFixtures.PASSWORD);
        authFactory.when(() -> AuthFactory.getStoredKey(anyString(), eq(ScramSha256SaslServer.MECHANISM_NAME))).thenReturn(DatatypeConverter.printBase64Binary(StringUtils.decodeHex(ScramSha256TestFixtures.STORED_KEY_HEX)));
        authFactory.when(() -> AuthFactory.getServerKey(anyString(), eq(ScramSha256SaslServer.MECHANISM_NAME))).thenReturn(DatatypeConverter.printBase64Binary(StringUtils.decodeHex(ScramSha256TestFixtures.SERVER_KEY_HEX)));
    }

    /**
     * Computes a valid SCRAM-SHA-256 client proof for a given SCRAM exchange state.
     *
    ...
    /**
     * Returns the username used throughout SCRAM-SHA-256 unit tests.
     *
    ...
    {
        return ScramSha256TestFixtures.USER;
    }

    /**
     * Returns the client nonce used throughout SCRAM-SHA-256 unit tests.
     *
    ...
    {
        return ScramSha256TestFixtures.CLIENT_NONCE;
    }

    /**
     * Returns the expected byte length of a SCRAM-SHA-256 client proof (32 bytes for SHA-256).
     *
    ...
    /**
     * Creates a new ScramSha256SaslServer instance configured for either the plain or PLUS variant.
     *
     * @param isPlusMechanism true to create a SCRAM-SHA-256-PLUS server, false for SCRAM-SHA-256
     * @return a new ScramSha256SaslServer
     */
    @Nonnull
    @Override
    protected ScramSha256SaslServer newServer(final boolean isPlusMechanism)
    {
        return new ScramSha256SaslServer(isPlusMechanism, new HashMap<>(), ScramSha256TestFixtures.SUPPORTED_MECHANISMS, ScramSha256TestFixtures.SUPPORTED_CHANNEL_BINDING_TYPES);
    }
     ...
     */
    protected ScramSha256SaslServer newServer(boolean isPlusMechanism, Set<String> advertisedMechanismNames)
    {
        return new ScramSha256SaslServer(isPlusMechanism, new HashMap<>(), advertisedMechanismNames, ScramSha256TestFixtures.SUPPORTED_CHANNEL_BINDING_TYPES);
    }

    /**
     * Creates a new ScramSha256SaslServer instance with specific advertised mechanisms and channel-binding types.
     *
     * @param isPlusMechanism true to create a SCRAM-SHA-256-PLUS server, false for SCRAM-SHA-256
     * @param advertisedMechanismNames The names of SASL mechanisms that are advertised to the peer.
    ...
    @Nonnull
    @Override
    protected ScramSha256SaslServer newServer(final boolean isPlusMechanism, final Set<String> advertisedMechanismNames, final Set<String> advertisedChannelBindingTypes)
    {
        return new ScramSha256SaslServer(isPlusMechanism, new HashMap<>(), advertisedMechanismNames, advertisedChannelBindingTypes);
    }

    /**
     * Returns the SHA-256 downgrade protection hash published in the worked example of XEP-0474 §6.3.
     */
    @Override
    protected String expectedDowngradeProtectionHash()
    {
        return ScramSha256TestFixtures.SSDP_HASH;
    }

    /**
     * Returns the SHA-256 downgrade protection hash over the example mechanism list alone.
     */
    @Override
    protected String expectedDowngradeProtectionHashWithoutChannelBindingTypes()
    {
        return ScramSha256TestFixtures.SSDP_HASH_WITHOUT_CHANNEL_BINDING_TYPES;
    }
    ...
    {
        // Setup test fixture
        final SecretKeyFactory pbkdf2Factory = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA256");
        setupCanonicalAuthData();

        // Setup test fixture: prepare initial client message.
        final ScramSha256SaslServer server = newServer(false);
        final String gs2Header = "n,,";
        final byte[] initialMessage = createClientInitialMessage(gs2Header, ScramSha256TestFixtures.USER, ScramSha256TestFixtures.CLIENT_NONCE);
        ...
        assertTrue(firstExchangeResult.serverNonce.startsWith(ScramSha256TestFixtures.CLIENT_NONCE), "First server message should contain a server nonce that starts with the client nonce, but did not.");
        assertArrayEquals(DatatypeConverter.parseBase64Binary(ScramSha256TestFixtures.SALT), firstExchangeResult.salt, "First server message should include the 'salt' value configured for this unit test (but did not)");
        assertEquals(ScramSha256TestFixtures.ITERATIONS, firstExchangeResult.iterations, "First server message should include the 'iterations' value configured for this unit test (but did not)");
        ...
        final KeySpec saltedPasswordSpec = new PBEKeySpec(ScramSha256TestFixtures.PASSWORD.toCharArray(), firstExchangeResult.salt, firstExchangeResult.iterations, expectedProofLengthBytes()*8); // SCRAM derives the salted password at the hash's native output length, which equals the proof length
        ...
        final byte[] serverKey = new HmacUtils(HmacAlgorithms.HMAC_SHA_256, saltedPassword).hmac(ScramSha256TestFixtures.SERVER_KEY);
        final byte[] serverSignature = new HmacUtils(HmacAlgorithms.HMAC_SHA_256, serverKey).hmac(authMessage);
    ...
    /**
     * Implements a successful SCRAM-SHA-256-PLUS exchange with channel binding.
     */
    @Test
    public void testSuccessPlus() throws Exception
    {
        ...
        final ScramSha256SaslServer server = new ScramSha256SaslServer(true, props, ScramSha256TestFixtures.SUPPORTED_MECHANISMS, ScramSha256TestFixtures.SUPPORTED_CHANNEL_BINDING_TYPES);
     ...
     */
    @Test
    void getMechanismName_returnsScramSha256_forNonPlusMechanism()
    {
        // Setup test fixture
        final ScramSha256SaslServer server = newServer(false);
        ...
        // Verify result
        assertEquals(ScramSha256SaslServer.MECHANISM_NAME, mechanismName, "Non-PLUS mechanism should return 'SCRAM-SHA-256' as its name");
    }
     ...
     */
    @Test
    void getMechanismName_returnsScramSha256Plus_forPlusMechanism()
    {
        // Setup test fixture
        final ScramSha256SaslServer server = newServer(true);

        // Execute system under test
        final String mechanismName = server.getMechanismName();

        // Verify result
        assertEquals(ScramSha256SaslServer.MECHANISM_NAME+"-PLUS", mechanismName, "PLUS mechanism should return 'SCRAM-SHA-256-PLUS' as its name");
    }
}
