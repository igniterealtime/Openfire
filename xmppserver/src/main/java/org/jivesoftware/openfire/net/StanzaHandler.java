...
public abstract class StanzaHandler {
     ...
     * separately, via {@link SASLAuthentication#SASL2_BIND2_NEGOTIATION_ACTIVE_OR_DONE} session data - see the guard in
    ...
    protected void processStanza(String stanza, XMPPPacketReader reader) throws Exception {
        ...
        if ("starttls".equals(tag)) {
            ...
            if (saslStatus == SASLAuthentication.Status.failed) {
                // Negotiation ended without success: the peer is permitted to start a new one (up to the configured
                // retry limit, enforced by SaslOutcome#authenticationFailed), so this is no longer 'in progress'.
                startedSASL = false;
                usingSASL2 = false;
            }
            if (saslStatus == SASLAuthentication.Status.authenticated && usingSASL2) {
                ...
                startedSASL = false;
                authenticatingSession.setSessionData(SASLAuthentication.SASL2_BIND2_NEGOTIATION_ACTIVE_OR_DONE, Boolean.TRUE);
