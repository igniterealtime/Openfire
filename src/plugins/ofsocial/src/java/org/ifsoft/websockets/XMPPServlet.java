...
import org.jivesoftware.openfire.plugin.ofsocial.OfSocialPlugin;
...
public final class XMPPServlet extends WebSocketServlet
{
    ...
    private OfSocialPlugin plugin;

    public XMPPServlet()
    {
		plugin = (OfSocialPlugin) XMPPServer.getInstance().getPluginManager().getPlugin("ofsocial");
				...
				try {
					...
					try {
						...
						if (username.equals("null") && password.equals("null"))	// anonymous user
						{
							...
							if (isExistingSession && wasAvailable == false && (password.equals("dummy") || password.equals("reuse")))
							{
