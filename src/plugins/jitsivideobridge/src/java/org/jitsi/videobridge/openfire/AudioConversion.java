...
{
    private static final Logger Log = LoggerFactory.getLogger(AudioConversion.class);
	...
	Log.debug("Time to generate l2u table " + (System.currentTimeMillis() - start));
    }
    ...
    public static void linearToUlaw(byte[] linearData, int linearOffset,
        ...
        {
            //ulawData[outIx++] = linearToUlaw((linearData[inIx] << 8) | (linearData[inIx + 1] & 0xff)); // hack for jitsivideobridge
            ulawData[outIx++] = linearToUlaw((linearData[inIx + 1] << 8) | (linearData[inIx] & 0xff));
        }
    }
    ...
    public static int clip(int sample) {
        if (sample > 32767) {
	        Log.debug("Clip " + sample + " to 0x7fff (32767)");
        ...
        if (sample < -32768) {
	    	...
	    	Log.debug("Clip " + sample + " to 0xffff8000 (-32768)");
