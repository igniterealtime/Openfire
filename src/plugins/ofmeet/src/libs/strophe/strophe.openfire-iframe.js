...
Openfire.Connection = function(url) 
{
    ...
    this.mode = "normal";
    if (window.parent && window != window.parent) this.mode = "child";
    
    console.log("Openfire Connection in " + this.mode);    
}
...
Openfire.Connection.prototype = {
    ...
    becomeParent: function (iFrameId)
    {
    	...
    	{
    		if (!that.iframe || !that.iframe.contentWindow)
    		{
    			that.iframe = document.getElementById(that.iFrameId);
    		}
    ...
    { 
        this.resource = jid.indexOf("/") > -1 ?  Strophe.getResourceFromJid(jid) :Strophe.getNodeFromJid(jid) + "-" + Math.round(Math.random() * 10000);       
	...
	try {
	   ...
	   this._changeConnectStatus(Strophe.Status.CONNECTED, null);	
	    ...
	    {	
	    	this.iframe.contentWindow.connection._onopen();
	    }
	...
	if (xml != " ")
	{
    	...
    	if (this.mode == "child") parent.postMessage(xml, '*');
    },
	...
	if (this._ws) this._ws.send(toSend);
    	...
    	if (this.mode == "child") parent.postMessage(toSend, '*');	
    },    
    ...
    {   
		...
		if (this.iframe && this.iframe.contentWindow)
		{	
			that.iframe = document.getElementById(this.iFrameId);
		} 
		
		if (this.iframe) this.iframe.contentWindow.postMessage(packet.data, "*");
	}
