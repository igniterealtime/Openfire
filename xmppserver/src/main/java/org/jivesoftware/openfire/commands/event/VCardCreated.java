...
public class VCardCreated extends AdHocCommand {
    ...
    public String getDefaultLabel() {
        return LocaleUtils.getLocalizedString("commands.event.vcardcreated.label");
    }
    ...
    public void execute(@Nonnull SessionData sessionData, Element command) {
        ...
        try {
            ...
            note.setText(LocaleUtils.getLocalizedString("commands.event.vcardcreated.note.username-required", preferredLocale));
            return;
        }
        ...
        if (vCard == null) {
            note.addAttribute("type", "error");
            note.setText(LocaleUtils.getLocalizedString("commands.event.vcardcreated.note.vcard-does-not-exist", preferredLocale));
            return;
        }
    ...
    protected void addStageInformation(@Nonnull final SessionData data, Element command) {
        ...
        form.setTitle(LocaleUtils.getLocalizedString("commands.event.vcardcreated.form.title", preferredLocale));
        form.addInstruction(LocaleUtils.getLocalizedString("commands.event.vcardcreated.form.instruction", preferredLocale));
        ...
        field.setLabel(LocaleUtils.getLocalizedString("commands.event.vcardcreated.form.field.username.label", preferredLocale));
