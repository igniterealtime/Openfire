...
public class LocalIncomingServerSessionTest
{
        ...
        throws Exception
    {
        ...
        try {
            ...
            RemoteInitiatingServerDummy.log("Setup fixture: remote server init");
            ...
            RemoteInitiatingServerDummy.log("Setup fixture: (done with setting up fixture)");

            // execute system under test.
            RemoteInitiatingServerDummy.log("Execute system under test: (start with execution)");
            ...
            RemoteInitiatingServerDummy.log("Execute system under test: starting connection listener");
            connectionListener.start();

            RemoteInitiatingServerDummy.log("Execute system under test: mocking connection manager");
            ...
            RemoteInitiatingServerDummy.log("Execute system under test: make the remote server connect.");
            remoteInitiatingServerDummy.connect(connectionListener.getPort());
            RemoteInitiatingServerDummy.log("Execute system under test: start connecting, block until done.");
            remoteInitiatingServerDummy.blockUntilDone(1, TimeUnit.MINUTES);
            RemoteInitiatingServerDummy.log("Execute system under test: done connecting.");

            RemoteInitiatingServerDummy.log("Execute system under test: get the incoming server session object.");
            ...
            RemoteInitiatingServerDummy.log("Execute system under test: (done with execution)");

            // Verify results
            RemoteInitiatingServerDummy.log("Verify results (start)");
            RemoteInitiatingServerDummy.log("Expect: " + expected.getConnectionState() + ", Result: " + result);
            ...
            RemoteInitiatingServerDummy.log("Expectation met.");
            RemoteInitiatingServerDummy.log("Verify results (done)");
        } finally {
            // Teardown test fixture.
            RemoteInitiatingServerDummy.log("Teardown test fixture (start)");
            trustStore.delete("unit-test");
            if (connectionListener != null) {
                connectionListener.stop();
            }
            RemoteInitiatingServerDummy.log("Teardown test fixture (done)");
        }
    }
