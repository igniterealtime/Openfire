    ...
    <tbody>
        <c:forEach var="field" items="${requestScope.fields}">
            <c:if test="${not requestScope.nonDisplayFields.contains(field.variable)}">
                <tr>
                    <td nowrap style="min-width: 300px"><label style="font-weight: bold" for="${field.variable}">${field.label}</label></td>
                    ...
                    <c:choose>
                        <c:when test="${field.type.name() eq 'boolean_type'}">
                            <td width="1%" rowspan="2"><input type="checkbox" name="${field.variable}" id="${field.variable}" ${field.firstValue == 1 ? 'checked="checked"' : '' } /></td>
                        </c:when>
                        <c:when test="${field.type.name() eq 'text_single'}">
                            <td width="1%" rowspan="2"><input type="text" name="${field.variable}" id="${field.variable}" value="${field.firstValue}" style="width: 200px;" /></td>
                        </c:when>
                        <c:when test="${field.type.name() eq 'list_single'}">
                            <td width="1%" rowspan="2"><select name="${field.variable}" id="${field.variable}" style="width: 200px;">
                                    <c:forEach var="option" items="${field.options}">
                                        <option value="${option.value}" ${option.value == field.firstValue ? 'selected' : '' }>
                        ...
                        <c:when test="${isList and not empty field.options}">
                            <td width="1%" rowspan="2"><select name="${field.variable}"
                                id="${field.variable}" style="width: 200px;" multiple>
                                    <c:forEach var="option" items="${field.options}">
                                        <option value="${option.value}" ${ field.values.contains(option.value) ? 'selected' : '' }>
                                        ${option.label ? option.label : option.value }
                                        </option>
                                    </c:forEach>
                            </select>
                                <button type="button" onclick="clearSelected('${field.variable}')">
                        ...
                        <c:when test="${isList and empty field.options}">
                            <td rowspan="2">
                                <div class="jive-table">
                                    <table id="${field.variable}" cellpadding="0" cellspacing="0" border="0" width="100%">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            <c:forEach var="value" items="${field.values}" varStatus="loop">
                                                <tr id="${field.variable}${loop.index}">
                                                    <td><input type="hidden" name="${field.variable}" value="${value}" />${value}</td>
                                                    <td><button type="button" onclick="deleteTableRow('${field.variable}${loop.index}')">Remove</button></td>
                                                </tr>
                                            </c:forEach>
                                            <tr>
                                                <td><input type="text" style="width: 200px;" id="${field.variable}-Additional" name="${field.variable}-Additional" onkeypress="detect_enter_keyboard(event)" /></td>
                                                <td><input type="submit" id="${field.variable}-Add" name="${field.variable}-Add" value="<fmt:message key="global.add" />"></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
